...
public final class QuestionChooser extends ComponentBase implements IQuestionChooser {
	private Hashtable<String, IQuestion> hash;
	 ...
	 */
	public Hashtable<String, IQuestion> getHashtable() {
		return this.hash;
	}
	...
	public QuestionChooser() {
		this.hash = new Hashtable<String, IQuestion>();
	...
	private final Vector<IQuestion> popQuestions(IBaseConhecimento base, String[] animals) {
		...
		if (this.IsUsingMultithreading()) {
			PizzaThreadPool pool = new PizzaThreadPool(10);
			
			for (int animal = 0; animal < animals.length; animal++) {
				obj = base.recuperaObjeto(animals[animal]);
				pool.EnqueueThread(new PizzaThread(obj, this.hash, questions));
			}
		}
		else {
			for (int animal = 0; animal < animals.length; animal++) {
	...
	public final String BestQuestion (IPossibleAnimalsHash hash){
		String animals[] = hash.getPossibleAnimalsArray();
		IBaseConhecimento base = new BaseConhecimento();
		
		return (BestQuestion (base, animals));	
	}
