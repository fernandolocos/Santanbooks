...
@Component(id = "<http://purl.org/dcc/pt.c03ensaios.fabsouza.FileNaxxramasGUI.FileNaxxramasGUI>", 
		provides = { "<http://purl.org/dcc/pt.c03ensaios.fabsouza.FileNaxxramasGUI.IFileNaxxramas>" }, 
		requires = { "<http://purl.org/dcc/pt.c03ensaios.hayasida.handler.IHandler>",
					 "http://purl.org/dcc/pt.c03ensaios.naxxramas.INaxxramas>" })
					 
public class FileNaxxramasGUI extends ComponentBase implements IFileNaxxramas, IRequires<INaxxramas> {
	...
	private void initialize() {
		pnlFundo = getJPanelFundo();
		menuArquivo = getJMenuArquivo();
		menuAjuda = getJMenuAjuda();
	}
	...
	public void connect(INaxxramas arg0) {
		arg0.add(menuArquivo);
		arg0.add(menuAjuda);
		arg0.add("FileNaxxramas", pnlFundo);
	}
	 ...
	 * 
	 */
	private void messageSucesso() {
		JOptionPane.showMessageDialog(pnlFundo,
				"Animal cadastrado com sucesso.", "Sucesso",
				JOptionPane.INFORMATION_MESSAGE);
	}

	/**
	 * Exibe mensagem avisando o usuário de que o animal que está tentando
	 * cadastrar já existe.
	 * 
	 */
	private void messageJaExiste() {
		JOptionPane
				.showMessageDialog(
						pnlFundo,
						"Já existe um animal cadastrado com as mesmas perguntas e respostas.",
	...
	private class Sobre implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			lblNome = new JLabel("FileNaxxramas");
